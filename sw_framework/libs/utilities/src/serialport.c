...
static SerialPort SerialPort_obj =
{
    SerialPort_sendFrameCommand,
  ...
  if (buffer != NULL)
  {
    ...
    do
    {
      ...
      if (rc)
      {
        ...
        if (rc)
        {
          rc = SerialPort_receiveByte (&crc)
              && (crc == SerialPort_CRC (buffer, *size));
        }
...
static uint8_t SerialPort_sendFrameCommand(uint8_t * command,
                                           uint8_t cmdSize,
                                           uint8_t * data,
                                           uint8_t dataSize)
{
  ...
  if ((command != NULL) && ((data != NULL) || (dataSize == 0)))
  {
    //uint8_t crc_r = 0;
    uint8_t attempts = 0;
    uint8_t crc = SerialPort_CRC (command, cmdSize)
        + SerialPort_CRC (data, dataSize);
    do
    {
      //SerialPort_purge();
      SerialPort_sendByte (SerialPort_frameSignature);
...
static uint8_t SerialPort_receiveByte(uint8_t * byte)
{
  ...
  {
    *byte = (uint8_t) XUartPs_ReadReg(SerialPort_stdinAddress,
                                      XUARTPS_FIFO_OFFSET);
    rc = 1;
  }
